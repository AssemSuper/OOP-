...
public class TeacherTest implements Serializable{
	...
	private static void getCourses() throws IOException {
		...
		for(int i=0;i<t.getCourse().size();i++) {
			System.out.println((i+1)+". "+t.getCourse().get(i).getName());
		}	
	...
	private static void putMark(String it) throws IOException {
		for(int i=0;i<t.getCourse().get(Integer.parseInt(it)-1).getStudents().size();i++) {
			System.out.println((i+1)+". "+t.getCourse().get(Integer.parseInt(it)-1).getStudents().get(i).getFirstname()+" "+
					t.getCourse().get(Integer.parseInt(it)-1).getStudents().get(i).getLastname());
		}
	...
	private static void putMarkStudent(int indexOfSubject, int indexOfStudent) throws IOException {
		...
		HashMap<Discipline, Mark> mark = new HashMap<Discipline, Mark>();
		
		System.out.println(m);
		System.out.println(t.getDiscipline().get(indexOfSubject));
		mark.put(t.getCourse().get(indexOfSubject), m);
		System.out.println(mark);
		
		
		t.getCourse().get(indexOfSubject).getStudents().get(indexOfStudent).setPoints(mark);///ERROR
		
		Database.serialize("student");
		Database.serialize("teacher");
		Database.serialize();
	...
	private static void getInfoStudent(String it) throws IOException {
		...
		//System.out.println("-----------------------");
		for(int i=0;i<t.getDiscipline().get(Integer.parseInt(it)-1).getStudents().size();i++) {
			System.out.println((i+1)+". "+t.getCourse().get(Integer.parseInt(it)-1).getStudents().get(i).getFirstname()+" "+
					t.getCourse().get(Integer.parseInt(it)-1).getStudents().get(i).getLastname()+" has Point out of 100 -> "+
					t.getCourse().get(Integer.parseInt(it)-1).getStudents().get(i).getPoints().get(t.getCourse().get(Integer.parseInt(it)-1)));
		  			System.out.println("----------------------------");
		  			System.out.println(t.getCourse().get(Integer.parseInt(it)-1).getStudents().get(i).getPoints());
		  			System.out.println("----------------------------");
